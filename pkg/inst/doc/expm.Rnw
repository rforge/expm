...
The \pkg{expm} packages provides an \proglang{R} function \code{expm}
...
The \proglang{R} function \texttt{expm} takes in argument a real,
...
\cite{Ward:77} based on diagonal Padé table approximations in
conjunction with three step preconditioning. The Padé approximation to
$e^{\mat{A}}$ is
\begin{displaymath}
